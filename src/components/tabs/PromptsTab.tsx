...
export function PromptsTab({ skills }: PromptsTabProps) {
  ...
  return (
    <main className="w-full h-full p-6 bg-white dark:bg-zinc-950 overflow-y-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800/80 pb-5">
          <div>
            <h2 className="text-base font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
              <Terminal weight="light" className="w-5 h-5 text-emerald-500" />
          ...
          <div className="relative w-full sm:w-72">
            ...
            <Input
              ...
              className="pl-8 text-xs bg-zinc-50 dark:bg-zinc-900/90 border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100"
            />
          </div>
        </div>
                  ...
                  <CardHeader className="p-4 pb-2 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 font-mono text-xs text-orange-600 dark:text-orange-400 bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/20 px-2 py-1 rounded transition-colors cursor-pointer"
                      >
                        <span className="font-semibold">{trigger}</span>
                        {isCopied ? (
                          <Check weight="bold" className="w-3 h-3 text-emerald-500" />
                        ) : (
                          <Copy weight="light" className="w-3 h-3 text-orange-500/80 opacity-60 group-hover:opacity-100 transition-opacity" />
                        )}
                      </button>
                    ...
                    <CardTitle className="text-xs font-semibold text-zinc-900 dark:text-zinc-200 truncate">
                      {skill.name}
                    </CardTitle>

                    <CardDescription className="text-[11px] text-zinc-600 dark:text-zinc-400 line-clamp-2 leading-relaxed">
                    ...
                    <div className="flex items-center justify-between text-[10px] text-zinc-500">
                      ...
                      {skill.metadata.tools && skill.metadata.tools.length > 0 && (
                        <div className="flex items-center gap-1 text-zinc-500 dark:text-zinc-400 font-mono">
                          <Wrench weight="light" className="w-3 h-3 text-zinc-400 dark:text-zinc-500" />
